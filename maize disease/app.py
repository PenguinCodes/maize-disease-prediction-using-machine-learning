...
def maize_predict_diseases(pre_list):
    X = week_df[[ 'Max Temperature ','Min Temperature', 'Maximum RH', 'Minimum RH', 'Rainfall']]
    y = week_df['Class Label']
...
def predict_diseases():
    ...
    if to_predict_list:
        ...
        res = maize_predict_diseases(to_predict_list)
        if res:
...
def prdict_image(img_path):
    ...
    ddd=" "
    if preds==0:
        ddd="leaf is affected with blight spot"
    elif preds==1:
        ddd="leaf is affected with common rust"
    elif preds==2:
        ddd="leaf is affected with gray spot"
    else:
        ddd="leaf is healthy"
